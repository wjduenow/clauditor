...
| 3 | `EvalSpec.transport` field | Declared in the skill's `eval.json`. Same three values. Defaults to `auto` when the field is absent or explicitly `null`. | Per-skill author preference — e.g. a research skill that routinely exhausts the API tier can declare `"transport": "cli"` so every run uses subscription auth by default. |
...
- **CLI transport ignores `max_tokens` and `model` routing via the CLI branch.** `call_anthropic(prompt, model=..., max_tokens=..., transport="cli")` accepts the same signature for parity but the `max_tokens` and `model` arguments are **not** forwarded to the `claude -p` subprocess (the CLI binary has its own model resolution and response-length handling). Callers that need a specific model for a specific call must force `--transport api`.
