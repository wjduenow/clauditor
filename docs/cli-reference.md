...
| `--cross-harness` | Allow comparing across mixed harnesses (`claude-code` vs `codex`). Delta mode only — `compare --blind` is unaffected. When the two compared inputs are both loadable as `GradingReport` (positional `.grade.json` paths or `--skill --from --to` iteration directories) and their `harness` metadata differs, `clauditor compare` refuses with exit `2` unless this flag is passed. With the flag, a single-line `WARNING: comparing across harnesses ...` prints to stderr and the diff proceeds. Silent-skip when either input is a `.txt` capture (no harness metadata available). |
