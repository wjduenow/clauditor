...
| `--grading-provider {anthropic,openai,auto}` | Select which provider's SDK handles the LLM-grader call. Precedence on `propose-eval` specifically: this flag > `CLAUDITOR_GRADING_PROVIDER` env > default `auto` (the eval-spec-field layer does not apply since `propose-eval` runs before an eval spec exists). Under `auto`, the inference layer maps `claude-*` model prefix → `anthropic` and `gpt-*` / `o[0-9]+*` → `openai`; when no model is available the resolver falls back to anthropic (subscription-first historical default). Same flag on `grade`, `extract`, `triggers`, `compare --blind`, `suggest` (those commands also honor `EvalSpec.grading_provider`). See [docs/eval-spec-reference.md#optional-top-level-fields](eval-spec-reference.md#optional-top-level-fields) for the spec-field semantics. |
