"""Anthropic provider backend — SDK + CLI transport seam.
...
- returns a :class:`ModelResult` bundling the joined text,
...
Module location: this module is the canonical home of the SDK seam
post-#144 US-002. The shim :mod:`clauditor._anthropic` re-exports
every public name here for one release of back-compat. Class identity
is preserved (`is` returns True for re-exports).
"""
...
# sub-seam already lives in ``clauditor._providers._auth`` (US-001).
# Re-import here so an existing ``from clauditor._providers._anthropic
# import AnthropicAuthMissingError`` resolves cleanly. Class identity
# is preserved — the class is defined exactly once, in
# ``clauditor._providers/__init__.py``.
...
@dataclass
class ModelResult:
    """Bundle returned by :func:`call_anthropic` (and future :func:`call_model`).

    Renamed from ``AnthropicResult`` in #144 US-002 to support the
    multi-provider dispatcher landing in US-003. The legacy name
    ``AnthropicResult`` is kept as a module-level alias
    (``AnthropicResult = ModelResult``) so existing callers and
    docstrings keep resolving without modification.
    ...
    Attributes:
        ...
        duration_seconds: Wall-clock time the successful attempt
            ...
            duration (DEC-020).
        provider: Which provider backend produced this result. Defaults
            to ``"anthropic"`` for back-compat with every
...
# as a synonym for ``ModelResult`` so existing tests, docstrings, and
# imports (``from clauditor._anthropic import AnthropicResult``) keep
...
def _extract_result(response: Any) -> ModelResult:
    """Project an SDK response into a :class:`ModelResult`.
...
async def call_anthropic(
    ...
    *,
    ...
    See module docstring for the retry policy. On success returns a
    :class:`ModelResult`; on any non-retriable or retry-exhausted
...
def _build_default_harness():
    ...
    ``clauditor._providers._anthropic`` does eagerly import
...
async def _call_via_claude_cli(
    ...
    *,
    ...
    :class:`clauditor.runner.InvokeResult` onto :class:`ModelResult`.
