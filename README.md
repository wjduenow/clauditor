...
Note: `--no-api-key` only affects the subprocess; five commands (`grade`, `propose-eval`, `suggest`, `triggers`, `extract`) still call the Anthropic API from the Python process and require `ANTHROPIC_API_KEY` — a Pro/Max subscription alone does not grant API access. See [Authentication and API Keys](docs/cli-reference.md#authentication-and-api-keys) for the full list and exit-2 behavior (follow-up #86).
