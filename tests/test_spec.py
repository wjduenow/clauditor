...
class TestSkillSpecTransportThread:
    """DEC-012 / DEC-017 of #86: transport precedence through ``SkillSpec.run``.

    The run method reads ``os.environ["CLAUDITOR_TRANSPORT"]`` at call
    time and funnels all three layers through ``resolve_transport``.
    The winning value is stored on ``self.resolved_transport`` so
    grader callers can thread it to ``call_anthropic``.

    Precedence: CLI override > env var > eval_spec.transport > default "auto".
    """

    def test_cli_override_wins(self, tmp_skill_file, mock_runner, monkeypatch):
        """CLI ``--transport`` beats env var and spec field."""
        ...
        runner = mock_runner(output="ok")
        spec = SkillSpec.from_file(skill_path, runner=runner)
        spec.run(transport_override="api")
        assert spec.resolved_transport == "api"

    def test_env_wins_when_no_cli_override(
        self, tmp_skill_file, mock_runner, monkeypatch
    ):
        """``CLAUDITOR_TRANSPORT`` env beats spec field when CLI is unset."""
        ...
        skill_path, _ = tmp_skill_file("env-wins", eval_data=eval_data)
        runner = mock_runner(output="ok")
        spec = SkillSpec.from_file(skill_path, runner=runner)
        spec.run()
        assert spec.resolved_transport == "cli"

    def test_spec_wins_when_no_cli_or_env(
        self, tmp_skill_file, mock_runner, monkeypatch
    ):
        """``eval_spec.transport="cli"`` wins when CLI + env both unset."""
        monkeypatch.delenv("CLAUDITOR_TRANSPORT", raising=False)
        eval_data = {
            "skill_name": "spec-wins",
            "test_args": "",
    ...
    def test_default_auto_when_none_set(
        self, tmp_skill_file, mock_runner, monkeypatch
    ):
        """All three layers unset → default ``"auto"``."""
        monkeypatch.delenv("CLAUDITOR_TRANSPORT", raising=False)
        eval_data = {
            "skill_name": "all-none",
            "test_args": "",
        ...
        skill_path, _ = tmp_skill_file("all-none", eval_data=eval_data)
        runner = mock_runner(output="ok")
        spec = SkillSpec.from_file(skill_path, runner=runner)
        spec.run()
        assert spec.resolved_transport == "auto"

    def test_no_eval_spec_defaults_to_auto(
        self, tmp_skill_file, mock_runner, monkeypatch
    ):
        """Direct-constructor path with ``eval_spec=None`` → "auto"
        unless CLI / env override sets otherwise.
        """
        monkeypatch.delenv("CLAUDITOR_TRANSPORT", raising=False)
        skill_path = tmp_skill_file("no-spec")
        runner = mock_runner(output="ok")
    ...
    def test_no_eval_spec_env_still_wins(
        self, tmp_skill_file, mock_runner, monkeypatch
    ):
        """Env var still applies even when ``eval_spec`` is ``None``."""
        monkeypatch.setenv("CLAUDITOR_TRANSPORT", "api")
        skill_path = tmp_skill_file("no-spec-env")
        runner = mock_runner(output="ok")
        spec = SkillSpec(skill_path=skill_path, eval_spec=None, runner=runner)
        spec.run()
        assert spec.resolved_transport == "api"
    ...
    def test_empty_env_treated_as_unset(
        ...
        """Empty/whitespace ``CLAUDITOR_TRANSPORT`` → treated as unset,
        so the spec field wins.
        """
        monkeypatch.setenv("CLAUDITOR_TRANSPORT", "   ")
        eval_data = {
            "skill_name": "empty-env",
            "test_args": "",
            "assertions": [],
            "transport": "cli",
        }
        skill_path, _ = tmp_skill_file("empty-env", eval_data=eval_data)
        runner = mock_runner(output="ok")
        spec = SkillSpec.from_file(skill_path, runner=runner)
        spec.run()
        assert spec.resolved_transport == "cli"
    ...
    def test_invalid_env_raises(
        ...
        """Invalid ``CLAUDITOR_TRANSPORT`` surfaces as ``ValueError`` from
        ``resolve_transport`` — the CLI layer maps to exit 2.
        """
        monkeypatch.setenv("CLAUDITOR_TRANSPORT", "sdk")
        skill_path = tmp_skill_file("bad-env")
        runner = mock_runner(output="ok")
        spec = SkillSpec(skill_path=skill_path, eval_spec=None, runner=runner)
        with pytest.raises(
            ValueError,
            match=r"CLAUDITOR_TRANSPORT must be one of",
        ):
            spec.run()
